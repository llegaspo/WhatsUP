...
} from "@/utils/schema/FacebookPageSchema";
...
export const facebookPageRouter = router({
  create: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        if (newFBPage) return { message: "FacebookPage created Successfully" };
        else return { message: "FacebookPage cannot be created" };
      } catch (e) {
  ...
  fetchAll: publicProcedure.query(async () => {
    return await prisma.fBPage.findMany();
  }),
  ...
  delete: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        if (!fbPageDelete) return { message: "No such facebook page." };
        return { message: "Facebook page has been removed successfully." };
      } catch (e) {
        throw new Error(
          "An error has occured while deleting the facebook page",
        );
      }
    }),
  ...
  update: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        if (!fbPageUpdate) return { message: "Cannot update facebook page." };
      } catch (e) {
        throw new Error(
          "An error has occured while updating the facebook page.",
        );
      }
    }),
});
