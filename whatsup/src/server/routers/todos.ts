...
export const todoRouter = router({
  create: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (newTodo) return { message: "To do created Successfully" };
        else return { message: "To do cannot be created" };
      } catch {
  ...
  fetchAll: publicProcedure.query(async () => {
    try {
      ...
      if (!todos) return { message: "To dos cannot be fetched." };
      return todos;
    } catch {
  ...
  delete: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        if (!todoDelete) return { message: "No such to do exist." };
  ...
  update: publicProcedure.input(EditToDoSchema).mutation(async ({ input }) => {
    try {
      ...
      if (!todoUpdate) return { message: "Cannot update to do." };
